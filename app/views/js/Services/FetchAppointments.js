document.addEventListener('DOMContentLoaded', function () {
    ...
    const addEditableListeners = () => {
        ...
        editableCells.forEach(cell => {
            cell.addEventListener('dblclick', function () {
                ...
                if (field === 'date') {
                    const dateValue = rawValue.split(' ')[0];
                    this.innerHTML = `<input name="date" type="date" value="${dateValue}" class="w-full text-center">`;
                } else if (field === 'time') {
                    const timeValue = rawValue.includes(':') ? rawValue.split(' ')[1] || rawValue : convertTo24Hour(rawValue);
                    this.innerHTML = `<input name="time" type="time" value="${timeValue}" class="w-full text-center">`;
                } else {
    ...
    const saveUpdatedData = (cell) => {
        ...
        console.log(`Saving updated data: ID=${appointmentId}, Field=${field}, New Value=${newValue}`);
